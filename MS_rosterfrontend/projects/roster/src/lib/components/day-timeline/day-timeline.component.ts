import { Component, OnInit, ViewChild, Input, OnChanges, SimpleChanges, EventEmitter, Output, ChangeDetectionStrategy, ChangeDetectorRef } from '@angular/core';
...
import { LoaderService } from 'projects/lhs-service/src/lib/service/loader/loader.service';
...
@Component({
  selector: 'lib-day-timeline',
  templateUrl: './day-timeline.component.html',
  styleUrls: ['./day-timeline.component.scss']
})

export class DayTimelineComponent implements OnInit, OnChanges {
  ...
  constructor(private rosterService: RosterService, private loaderService: LoaderService,
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (this.shiftModel !== undefined) {
      for (let i = 0; i < this.shiftModel.length; i++) {
        for (let j = 0; j < this.shiftModel[i].employeeShiftInfoViewModel.length; j++) {
          ...
          // this.shiftMod.color = this.getShiftColor(this.shiftModel[i].statusName.toLowerCase());
          this.shiftMod.color = '#698063';
          this.shiftMod.text = this.shiftModel[i].description;
          this.shiftMod.status = this.shiftModel[i].statusName.toLowerCase();
    ...
    if (this.empModel.length > 0) {
      ...
      this.configDay.resources = this.employeeArray;
  ...
  configDay: DayPilot.SchedulerConfig = {
    // viewType: "Days",
    cellWidthSpec: "Fixed",
    cellWidth: 100,
    timeHeaders: [
  ...
  previous() {
    this.configDay.startDate = new DayPilot.Date(this.configDay.startDate).addDays(-1);
    this.dayVal = this.configDay.startDate.toString('dd/MM/yyyy');
  }
  ...
  showToday() {
    this.configDay.startDate = DayPilot.Date.today();
    this.dayVal = this.configDay.startDate.toString('dd/MM/yyyy');
    this.getAllShiftsDateWise(this.scheduler.control.visibleStart, this.scheduler.control.visibleStart);
  }
  ...
  getAllShiftsDateWise(startDate, endDate) {
    ...
    this.rosterService.GetEmployeeViewCalendar(data).subscribe((res: any) => {
      ...
      if (this.searchData.searchByEmpName > 0) {
        this.configDay.resources = this.employeeArray.filter(x => x.id == this.searchData.searchByEmpName);
      } else {
        this.configDay.resources = this.employeeArray;
      }
      switch (this.responseModel.status) {
