...
@Component({
  selector: 'lib-monthly-timeline',
  templateUrl: './monthly-timeline.component.html',
  styleUrls: ['./monthly-timeline.component.scss']
})

export class MonthlyTimelineComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  @Output() runningMonth = new EventEmitter<string>();
  employeeArray: any[] = [];
  ...
  navigatorDate: DayPilot.Date = new DayPilot.Date();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    //       this.shiftMod.status = this.shiftModel[i].statusName;
  ...
  getAllShiftsDateWise(startDate, endDate) {
    ...
    this.rosterService.GetEmployeeViewCalendar(data).subscribe((res: any) => {
      ...
      if (this.searchData.searchByEmpName > 0) {
        this.config.resources = this.employeeArray.filter(x => x.id == this.searchData.searchByEmpName);
      } else {
        this.config.resources = this.employeeArray;
      }
      switch (this.responseModel.status) {
  ...
  searchMonthlyShifts(data: searchFilters) {
  ...
  dateChange() {
    ...
    this.month = this.config.startDate.getMonth();
    this.getMonth(this.month);
    this.monthVal = this.monthVal + ' ' + this.year;
    this.runningMonth.emit(this.monthVal);
  }
