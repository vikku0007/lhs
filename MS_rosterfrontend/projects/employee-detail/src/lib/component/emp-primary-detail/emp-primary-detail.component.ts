...
@Component({
  selector: 'lib-emp-primary-detail',
  templateUrl: './emp-primary-detail.component.html',
  styleUrls: ['./emp-primary-detail.component.scss']
})
export class EmpPrimaryDetailComponent implements OnInit, OnChanges {
  ...
  selectedType: any;
  selectedother: any;
   constructor(private route: ActivatedRoute, private fb: FormBuilder, private empService: EmpDetailService,
  ...
  private filteroletype() {
    ...
    if (search.length >= 1) {
     this.filteredRecords.next(
      this.RoleList.filter(ServiceType => ServiceType.codeDescription.toLowerCase().indexOf(search) > -1)
     );
    }
  }
