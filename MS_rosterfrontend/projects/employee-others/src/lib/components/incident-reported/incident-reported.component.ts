...
import { AccidentIncidentDetails } from '../../view-models/accident-incident-details';
import { OtherService } from '../../services/other.service';

@Component({
  selector: 'lib-incident-reported',
...
export class IncidentReportedComponent implements OnInit {
  ...
  dataSourceAccidentIncident: any;
  totalCount: number;
  paging: Paging = {};
  orderBy: number;
  orderColumn: number;
  ...
  empAIModel: AccidentIncidentDetails = {};
  rForm: FormGroup;
  rForm1: FormGroup;
  searchByName = null;
  searchByType = null;
  ...
  EditList: AccidentIncidentDetails[] = [];
  EditaccidentId: any;
  EventTypeList: any;
  LocationList:any;
  accidentIncidentDetails: AccidentIncidentDetails[] = [];
  ...
  constructor(private route: ActivatedRoute, private otherService: OtherService,private fb: FormBuilder,
  ...
  ngOnInit(): void {
    this.dataSourceAccidentIncident = new MatTableDataSource(this.accidentIncidentDetails);
    this.getEmpIncidentList();
    this.createForm();
    this.createFormEdit();
    this.getEventtype();
     this.getReportedTo();
  ...
  createForm() {
    this.rForm = this.fb.group({
     ...
     incidentTime: ['', Validators.nullValidator],
    });
  }
  createFormEdit() {
    this.rForm1 = this.fb.group({
     ...
     incidentTimeEdit: ['', Validators.nullValidator],
    }); 
  }
  get accidentDate() {
  ...
  getEmpIncidentList() {    
    ...
    this.otherService.getAccidentIncidentList(data).subscribe(res => {
      this.response = res;
      switch (this.response.status) {
        case 1:
          this.accidentIncidentDetails = this.response.responseData;
          this.dataSourceAccidentIncident = new MatTableDataSource(this.accidentIncidentDetails);
  ...
  updateaccidentinfo() {
    if (this.rForm1.valid) {
      const data = {
       ...
       this.otherService.EditAccidentIncidentList(data).subscribe(res => {
       this.response = res;
       switch (this.response.status) {
        case 1:
        this.getEmpIncidentList();
  ...
  openEditDetails(elem) {
    ...
    this.rForm1.controls['incidentTimeEdit'].setValue(elem.incidentTimeName=="12:00 AM"?"24:00":elem.incidentTimeName=="12:30 AM"?"00:30":elem.incidentTimeName=="01:00 AM"?"01:00":elem.incidentTimeName=="01:30 AM"?"01:30":
  ...
  DeleteAccidentInfo(event){
    ...
    this.otherService.DeleteAccidentDetails(this.AccidentInfoModel).subscribe((data: any) => {
      if (data.status == 1) {
        this.getEmpIncidentList();
  ...
  AddAccidentIncidentInfo() {  
    if (this.rForm.valid) {
      ...
      this.otherService.AddAccidentIncidentList(data).subscribe(res => {
      this.response = res;
      switch (this.response.status) {
      case 1:
            this.notification.Success({ message: this.response.message, title: null });
