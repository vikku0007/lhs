...
@Component({
  selector: 'lib-employee-payroll-list',
...
export class EmployeePayrollListComponent implements OnInit, AfterViewInit {
  ...
  getPayrollEmployeeDetail(employeeid) {
    ...
    this.payrollService.getEmployeeHoursDetails(Number(employeeid), moment(this.startdate).format('YYYY-MM-DD'), moment(this.enddate).format('YYYY-MM-DD'), true).subscribe((res) => {
  ...
  getPayrollEmployeeList() {   
    ...
    this.payrollService.getEmployeeHoursList(Number(this.employeeId), moment(this.startdate).format('YYYY-MM-DD'), moment(this.enddate).format('YYYY-MM-DD'), true).subscribe((res) => {
      this.responseModel = res;
      this.totalCount = this.responseModel.total;
      switch (this.responseModel.status) {
        case 1:
