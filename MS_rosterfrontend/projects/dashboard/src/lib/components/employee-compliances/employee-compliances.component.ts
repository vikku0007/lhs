...
@Component({
  selector: 'lib-employee-compliances',
  templateUrl: './employee-compliances.component.html',
  styleUrls: ['./employee-compliances.component.scss'],
  providers: [
    {
...
export class EmployeeCompliancesComponent implements OnInit {
 ...
 private filtercompliances() {
    if (!this.MandatoryInfo) {
      return;
    }
    let search = this.control.value;
    if (!search) {
      this.filteredRecords.next(this.MandatoryInfo.slice());
      return;
    } else {
    ...
    if (search.length >= 1) {
     this.filteredRecords.next(
      this.MandatoryInfo.filter(department => department.codeDescription.toLowerCase().indexOf(search) > -1)
     );
    }
  }
  gettraining(event) { 
  ...
  getOptionalList() {
    this.commonService.getOptionalTraining().subscribe((res => {
      if (res) {
        this.response = res;
        this.MandatoryList = this.response.responseData || [];
        this.MandatoryInfo = this.response.responseData || [];
