...
@Component({
  selector: 'lib-employee-payrates',
  templateUrl: './employee-payrates.component.html',
  styleUrls: ['./employee-payrates.component.scss']
})
export class EmployeePayRateseComponent implements OnInit, OnChanges {
  ...
  constructor(private fb: FormBuilder, private route: ActivatedRoute, private commonService: CommonService, private empService: EmpServiceService, private notificationService: NotificationService) { }

  ngOnChanges(changes: SimpleChanges): void {
    if (this.empPayRates !== undefined) {
      this.route.queryParams.subscribe(params => {
        this.empPayRates.employeeId = Number(params.Id);
      });
  ...
  selected() {
    if (Number(this.level.value && this.level !== null)) {
  ...
  getEmployeelevel() {
    this.commonService.getEmployeelevel(this.empPayRates.employeeId).subscribe((res => {
      if (res) {
        this.response = res;
        this.isEmployeeLevel = this.response.responseData;
