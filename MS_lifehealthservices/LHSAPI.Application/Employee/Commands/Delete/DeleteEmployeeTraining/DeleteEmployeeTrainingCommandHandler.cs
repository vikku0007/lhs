...
namespace LHSAPI.Application.Employee.Commands.Delete.DeleteEmployeeTraining
{
    public class DeleteEmployeeTrainingCommandHandler : IRequestHandler<DeleteEmployeeTrainingCommand, ApiResponse>
    {
        ...
        public DeleteEmployeeTrainingCommandHandler(LHSDbContext context, ISessionService ISessionService)
        {
            _context = context;
        ...
        public async Task<ApiResponse> Handle(DeleteEmployeeTrainingCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                if (request.Id > 0)
                {
                    ...
                    var ExistEmp = _context.EmployeeTraining.FirstOrDefault(x => x.Id == request.Id && x.IsDeleted == false && x.IsActive == true);
                    if (ExistEmp == null)
                    {
                        response.NotFound();
                    }
                    else
                    {
                        ...
                        _context.EmployeeTraining.Update(ExistEmp);
