...
{
    public class GetEmployeeTotalWarningHandler : IRequestHandler<GetEmployeeTotalWarningQuery, ApiResponse>
    {
        private readonly LHSDbContext _dbContext;
        //   readonly ILoggerManager _logger;
        public GetEmployeeTotalWarningHandler(LHSDbContext dbContext)
        {
        ...
        /// <returns></returns>
        public async Task<ApiResponse> Handle(GetEmployeeTotalWarningQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var empList = (from emp in _dbContext.EmployeeJobProfile
                               ...
                               select new
                               {
                                   ...
                                   Totalwarning=_dbContext.EmployeeStaffWarning.Where(x => x.IsActive == true && x.IsDeleted == false && x.EmployeeId == request.EmployeeId).Count(),
                                   Todaydate=Newdate
                               }).ToList();
